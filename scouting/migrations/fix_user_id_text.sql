-- Fix: Use TEXT for user IDs since Google IDs exceed BIGINT max value
-- Google ID 105961375496902438796 > BIGINT max 9223372036854775807
...
-- Users table with auto-incrementing integer ID
CREATE TABLE users (
...
-- Accounts table - provider_account_id is TEXT to store large Google IDs
CREATE TABLE accounts (
    id SERIAL PRIMARY KEY,
    user_id INT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    type TEXT NOT NULL,
    provider TEXT NOT NULL,
    provider_account_id TEXT NOT NULL,  -- TEXT to handle Google's large IDs
    refresh_token TEXT,
    access_token TEXT,
    expires_at BIGINT,
    token_type TEXT,
