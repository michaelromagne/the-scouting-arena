-- Fix: Change user IDs from INTEGER to BIGINT to support large OAuth provider IDs
-- This fixes the error: "value is out of range for type integer"

-- Drop existing tables (safe since we're in development)
...
-- Recreate users table with BIGSERIAL
CREATE TABLE users (
...
CREATE TABLE user_activity (
    id BIGSERIAL PRIMARY KEY,
    user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    activity_type TEXT NOT NULL, -- 'view_player', 'compare_players', 'search', 'bookmark', etc.
...
-- User watchlists (collections of players)
